# Trainning/test_universal_mitosis_1008.py
...
config_file = str(project_root / 'configs/faster_rcnn_r50_test_midogpp.py')
checkpoint_file = str(project_root / 'outputs/work_dirs/faster_rcnn_r50_1008_40epochs/best_coco_bbox_mAP_epoch_4.pth')
img_root = project_root / 'data/Datensatz/MIDOG25_Track_FinalTest/'
ann_file = str(project_root / 'data/Datensatz/MIDOG25_Track_FinalTest/midog2025_patches_coco.json')

print(f"Project root identified as: {project_root}")
print(f"Checking if config exists: {os.path.exists(config_file)}")

IOU_THR = 0.5

SCORE_THRESHOLDS = np.round(np.arange(0.0, 0.96, 0.05), 2).tolist()

out_dir = Path(project_root / 'outputs/test/faster_rcnn_r50_1008_40epochs')
